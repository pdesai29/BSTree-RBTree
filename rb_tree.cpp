 ...
 */
  ...
  while (x != T_nil)
  {
    ...
    {
      // returning without inserting if key is already in the tree
      t_info.i_duplicate++;
      return;
    }
  }
 ...
 */
  ...
  // added counter increment for i_left_rotate,i_case_1,i_case_2,i_case_3,i_right_rotate
  rb_tree_node* y;
 ...
 */
  cout<<"Printing rb_tree in BFS form:"<<endl;
...
//helper to convert
int rb_tree::inorder_Traverse(rb_tree_node* x, int* array) {
  if (x != T_nil) {
    inorder_Traverse(x->left, array);
    array[counterRB] = x->key;
    counterRB++;

    inorder_Traverse(x->right, array);
  }
}
// implemented convert method for returning new number of nodes after discarding duplicate keys
...
//check_black_height travels right and left from passed node and returns 1 if black heigth is same on both side else returns -1
int rb_tree::check_black_height(rb_tree_node* x)
{
  //print key of nodes visited
  //cout<<x->key<<endl;
  ...
  // recursively checking black height of the left & the right subtrees from passed node
  int LBH = check_black_height(x->left);
  int RBH = check_black_height(x->right);
  
  // if LBH != RBH returning -1
  if (LBH != RBH) {
    return -1;
  }
  // If passed node is black node than adding 1 to LBH or RBH
  if (x->color == RB_BLACK) {
    LBH++;
  }
  // returning black height of the node
  return LBH;
  }
}
